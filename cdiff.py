...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, classes, shape, noise=None, cond_scale = 3.):
    ...
    @torch.no_grad()
    def ddim_sample(self, classes, shape, noise = None, cond_scale = 3., clip_denoised = True):
    ...
    @torch.no_grad()
    def sample(self, classes, noise = None, cond_scale = 3.):
        batch_size, image_size, channels = classes.shape[0], self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(classes, (batch_size, channels, image_size, image_size), noise, cond_scale)
    ...
    def p_losses(self, x_start, t, *, classes, noise = None):
        ...
        noise = default(noise, lambda: torch.randn_like(x_start)) if noise is None else noise
